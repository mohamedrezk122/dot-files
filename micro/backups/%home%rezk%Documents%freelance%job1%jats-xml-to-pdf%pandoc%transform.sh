...
pandoc -f jats-auto_identifiers "./mnt/$input" --standalone -o "./mnt/$output_name-intermediate.tex" -t latex #--filter ./abstract.sh

# xml -> latex -> pdf, using custom intermediate template
# often succeeds when direct xml->pdf doesnt
pandoc \
    -f latex-auto_identifiers "./mnt/$output_name-intermediate.tex" \
    ...
    --template="latex-template.tex" || {
        ...
        pandoc \
            -f latex-auto_identifiers "./mnt/$output_name-intermediate2.tex" \
            -o "./mnt/$output_name.pdf" \
            --pdf-engine=xelatex \
            --template="latex-template.tex"
}
